...
## Preparation BEFORE Sonar Java training
* Grant Git access to **prj_ELCAVN-TECH-TRAINING** for the list of participant at https://intranet.svc.elca.ch/corp/bl/VN/Training/Trainer/Documents/Forms/AllItems.aspx or Outlook invitation
* Follow **Option 2** (Copy and use **Searching in Active Directory.lnk**) in https://sdkb.svc.elca.ch/display/FAQ/Active+Directory+-+How+to+manage+AD+group
* You have to clone this repo and run two commands below to delete & re-create 12 bugfix branches in **Git Bash**

```
git branch -r | awk -F/ '/\/bugfix/{print $2"/"$3}' | xargs -I % git push origin --delete %
for INC in {1..12}; do git checkout -b bugfix/ELCAVNPAL-369-code-quality-sonar$INC master; git push -u origin bugfix/ELCAVNPAL-369-code-quality-sonar$INC; done
```
